...
server <- function(input, output) {
  ...
  {
    df<- do.call("rbind", parallel::parLapply(twtList,as.data.frame,mc.cores = numCores))
  ...
  score.sentiment <- function(sentences, pos.words, neg.words, .progress='none')
  {
    require(plyr)
    require(stringr)
    list=parallel::parLapply(sentences, function(sentence, pos.words, neg.words)
    {
    ...
    score_new=parallel::parLapply(list, `[[`, 1, mc.cores = numCores)
    pp1=score=parallel::parLapply(list, `[[`, 2, mc.cores = numCores)
    nn1=score=parallel::parLapply(list, `[[`, 3, mc.cores = numCores)
