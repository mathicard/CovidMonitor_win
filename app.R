...
pacman::p_load(shiny, shinydashboard, twitteR, rtweet, ROAuth, RCurl, stringr, ggplot2, glue, 
               httpuv, dplyr, purrr, reshape, tm, plotrix, plotly,
...
server <- function(input, output) {
  ...
  {
    df<- do.call("rbind", parallel::lapply(twtList,as.data.frame,mc.cores = numCores))
  ...
  score.sentiment <- function(sentences, pos.words, neg.words, .progress='none')
  {
    require(plyr)
    require(stringr)
    list=parallel::lapply(sentences, function(sentence, pos.words, neg.words)
    {
    ...
    score_new=parallel::lapply(list, `[[`, 1, mc.cores = numCores)
    pp1=score=parallel::lapply(list, `[[`, 2, mc.cores = numCores)
    nn1=score=parallel::lapply(list, `[[`, 3, mc.cores = numCores)
