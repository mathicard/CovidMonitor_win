...
server <- function(input, output) {
  ...
  {
    df<- do.call("rbind", lapply(twtList,as.data.frame,mc.cores = numCores))
  ...
  score.sentiment <- function(sentences, pos.words, neg.words, .progress='none')
  {
    ...
    score_new=lapply(list, `[[`, 1, mc.cores = numCores)
    pp1=score=lapply(list, `[[`, 2, mc.cores = numCores)
    nn1=score=lapply(list, `[[`, 3, mc.cores = numCores)
